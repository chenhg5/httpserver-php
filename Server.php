...
if (count($argv) > 1 && !in_array($argv[1], [SINGLE_PROCESS_MODE,
        MULTI_PROCESS_MODE,
        MULTI_THREAD_MODE,
        SELECT_POLL_MODE,
        SINGLE_PROCESS_COROITINES_MODE,
        EPOLL_KQUEUE_MODE,
        MULTI_PROCESS_FIXED_MODE])
) {
    die("参数错误=。=");
}

// 单进程模型：阻塞请求
if (count($argv) == 0 || $argv[1] == SINGLE_PROCESS_MODE) {
    $i = 0;
    while (1) {
        // -1 永不超时
...
// 多进程固定进程抢占式模型 进程间通信
if ($argv[1] == MULTI_PROCESS_FIXED_MODE) {
    for ($i = 0; $i < 32; $i++) {
        if (pcntl_fork() == 0) {
            while (1) {
